...
#define IN "/home/wave/Desktop/comp1001/my_github/COMP1001/COMP1001-master/25_26_coursework/Report/question1/Linux/code_to_start/input_images/a15.pgm"
#define OUT "/home/wave/Desktop/comp1001/my_github/COMP1001/COMP1001-master/25_26_coursework/Report/question1/Linux/code_to_start/output_images/edge_detection15.pgm"
...
void edge_detection( ){
	...
	for (row = 1; row < N - 1; row++) {
		for (col = 1; col < M - 1; col++) {
			...
			output_image[M*row+col] = (unsigned char) sqrt(Gx*Gx + Gy*Gy); /* Calculate output_image strength		*/
  ...
  for (i=0; i<M; i++) {
	...
	if( fscanf (finput,"%d",&temp) == EOF)
	 ...
	 for (j=0; j<N; j++){
	  for (i=0; i<M; i++) {
	c=getc(finput);
	input_image[M*j+i]= (unsigned char) c;
	  }
	  }
	}
else {
